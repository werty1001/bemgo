...
Все основные настройки хранятся в одном файле **app/config.js**, если этого файла не будет, то будут использованы настройки по умолчанию. Такой подход позволяет использовать одну сборку для разных проектов и настраивать каждое приложение индивидуально.  
### Содержимое конфинга
```js
module.exports = {
  ...
  extnames: {
    templates: '.pug', // по умоланию '.html'
    scripts: '.js',    // по умоланию '.js'
    styles: '.styl',   // по умоланию '.scss'
  },
  ...
  build: {
    ...
    pugMap: 'app/blocks/map.pug', // путь (от корня) для генерации PUG карты
    
    globalStyle: 'app/blocks/global.scss', // путь (от корня) до глобального файла для стилей
...
В качестве шаблонизатора разметки можно использовать Pug / Twig или обычный HTML с [include'ом](https://www.npmjs.com/package/gulp-file-include).  
...
### Автоматическое подключение SVG символа
Если вы хотите, чтобы SVG символ был прямо в теле HTML страниц, то вам достаточно указать специальный комментарий:
...
<html>
  ...
  <body>
    <!-- BEMGO:symbol --> // тут будет содержимое SVG
    ...
  </body>
</html>
```
...
#### Выход есть!
В сборке есть автоматическое подключение всех PUG файлов в одном месте. Это полностью решает проблему, а вам достаточно указать путь куда писать карту файлов в [config.js](#настройки-приложения) и подключить ее себе в layout.
```js
// app/config.js
...
```
Теперь при создании CSS файла `npm run add header[.css]` его содиржимое будет таким:
```css
.header {}
```
