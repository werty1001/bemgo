...
Все основные настройки хранятся в одном файле **app/config.js**, если этого файла нет, то будут использованы настройки по умолчанию. Такой подход позволяет использовать одну сборку для разных проектов и настраивать каждое приложение индивидуально.  
...
Если вы хотите, чтобы SVG символ был прямо в теле HTML страницы, то вам достаточно указать специальный комментарий:
...
```
Скорее всего у вас есть какой-то общий файл с переменными или миксинами, но в условиях независимости блоков его придется импортировать в каждый стиль отдельно. Мягко говоря не самый лучший вариант, поэтому есть способ получше – вы можете указать определенный файл в настройках и он будет импортирован при сборке автоматически.
```js
// app/config.js
...
```
Классно правда? Раньше приходилось возиться с переменными для каждого препроцессора, а теперь приятный и понятный код.
### Для ретины
...
```
### Вектор
Одновременно с обычными спрайтами можно создавать и векторные, принцип тот же, только с SVG иконками:
```css
...
# SVG symbols
Помимо обычных спрайтов для стилей, можно использовать SVG символы в HTML, иконки для этого спрайта нужно хранить в отдельной папке блока (symbols). Для каждой иконки будет сгенерирован свой ID по формуле block-name__icon-name, например:
```
card/symbols/arrow.svg -> #card__arrow
```
### Подключение SVG спрайта
Есть несколько вариантов подключения SVG, можно встроить прямо на страницу, для этого в HTML вам нужно указать специальный комментарий:
```html
<!-- BEMGO:symbol -->
```
...
Если в коде страницы будет найдена хотя-бы одна SVG иконка, то сборка будет искать специальный блок symbol на основном уровне разработки (св-во **mainLevel** в [config.js](#настройки-приложения)), вы можете создать два файла `prepend.svg` и `append.svg` внутри этого блока и тогда содержимое этих файлов будет добавлено в SVG символ.
### Сборка спрайта
В режиме разработки все иконки будут добавлены в спрайт, однако в финальную сборку попадут только те иконки, которые есть в коде, также вы можете добавить специальный ключ @always перед расширением иконки, тогда она попадет в спрайт в любом случае:
```
...
* Нету. Но это вообще не проблема! Для хранения каких-то глобальных файлов можно использовать корневой блок, например **page** или **app**, у вас скорее всего такой будет.
